 ...
 * @param {Function} Solver This is the main fdq export symbol for actual solving
 ...
 * @property {boolean} [options.hashNames=true] Hash names in the output dsl (passed on to fdq if it comes to that)
...
function _preSolver(dsl, Solver, options, solveOptions) {
  ...
  if (solution) {
    console.error('<solved without fdq>');
    return solution;
  }
  if (state === $REJECTED) {
    console.error('<rejected without fdq>');
  ...
  if (!Solver) THROW('problem not solved and fdq not passed on... Burn!');
